## buy 1000 rupees only when a 30 or more day low is hit 
...
logger.info('-----------------------------------------')
logger.info(' Buy fixed only when a 30 or more day low is hit')
logger.info('-----------------------------------------')
...
class MyStrategy(bt.Strategy):
    def __init__(self):
        ...
        self.mystats = open('mystats_sipfixed30plus.csv', 'w')
    ...
    def next(self):
        ...
        if self.data.close[0] == self.lowest_price:
            #print((dt - self.buy_date).days)
            #print(max(((dt - self.buy_date).days/30),1)*1000)
